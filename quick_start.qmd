...
- Copy the `dot-env-template.txt` file to `.env`

```zsh
cp dot-env-template.txt .env
```

- Open the `.env` file in visual studio code (or any other text editor) and update the variables in it.

```zsh
code .env
```

_Make sure that the `DEFAULT_PREFIX=loke` as this gives the running backend the name of "loke" which will be used later_.  
...
_Note: For a more custom hardware configuration, you might have to copy the example hardware config file `executor-config.example.yml` into the `executor-config.yml` file and update the variables there in._
